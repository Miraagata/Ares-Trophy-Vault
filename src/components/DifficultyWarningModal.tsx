...
export const DifficultyWarningModal: React.FC<DifficultyWarningModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-zinc-950/85 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-amber-600/60 rounded-2xl max-w-lg w-full p-6 shadow-2xl text-zinc-100 relative animate-in fade-in zoom-in-95 duration-200">
        
        {/* Close Button */}
        <button
          onClick={onCancel}
          className="absolute right-4 top-4 text-zinc-400 hover:text-white p-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="bg-zinc-950/90 border border-zinc-800 rounded-xl p-3 mb-4 space-y-2 max-h-40 overflow-y-auto">
          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider block">
            {isPt ? "Troféu(s) de Dificuldade Elevada:" : "Detected Trophy/Trophies:"}
          </span>
          {highDiffTrophies.map((t) => (
            <div key={t.id} className="flex items-center gap-2 text-xs bg-zinc-900/80 p-2 rounded-lg border border-amber-900/40">
              <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />
              <div className="truncate">
                <span className="font-bold text-amber-100 block truncate">{t.name}</span>
                <span className="text-[10px] text-zinc-400 truncate block">{t.description}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-amber-950/40 border border-amber-800/50 rounded-xl p-3.5 mb-5 space-y-2 text-xs text-amber-100">
          ...
          <div className="bg-amber-900/30 p-2 rounded-lg border border-amber-700/40 text-[11px] font-semibold text-amber-200">
            {isPt
        ...
        <div className="flex items-center justify-end gap-3 pt-3 border-t border-zinc-800">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-xs font-black bg-amber-500 hover:bg-amber-400 text-zinc-950 shadow-lg transition-colors cursor-pointer"
          >
