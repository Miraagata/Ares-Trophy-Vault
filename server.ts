...
import { parseTROPCONF, parsePARAMSFO, parseTROPUSR } from "./src/lib/ps3Parser";
...
Do not include markdown blocks, just raw JSON.`;
  ...
  app.post("/api/parse-source-dat", upload.single("sourceDat"), async (req, res) => {
    try {
      ...
      const { extractAllTimestampsFromDAT } = await import("./src/lib/ps3Parser");
  ...
  app.post("/api/export-tropusr", async (req, res) => {
    try {
      ...
      const { updateTROPUSR, recalculateDatHash } = await import("./src/lib/ps3Parser");
